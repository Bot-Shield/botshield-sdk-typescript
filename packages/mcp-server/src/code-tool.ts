...
const prompt = `Runs JavaScript code to interact with the Bot Shield API.
...
async function run(client) {
  const response = await client.sdk.createVerificationLink({ return_url: 'https://yourapp.com/verified', user_email: 'user@example.com' });
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    BOTSHIELD_API_KEY: readEnv('BOTSHIELD_API_KEY') ?? client.apiKey ?? undefined,
    BOTSHIELD_AGENT_KEY: readEnv('BOTSHIELD_AGENT_KEY') ?? client.agentKey ?? undefined,
    BOT_SHIELD_BASE_URL:
      readEnv('BOT_SHIELD_BASE_URL') ?? readEnv('BOT_SHIELD_ENVIRONMENT') ?
        undefined
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      project_name: 'botshield-sdk',
      code,
      intent,
      client_opts: { environment: (readEnv('BOT_SHIELD_ENVIRONMENT') || undefined) as any },
...
const localDenoHandler = async ({
  ...
  try {
    const sdkPkgName = 'botshield-sdk';
  ...
  try {
    const resp = await new Promise<Response>((resolve, reject) => {
      ...
      const opts = {
        ...
        ...(client.agentKey != null ? { agentKey: client.agentKey } : undefined),
        defaultHeaders: {
