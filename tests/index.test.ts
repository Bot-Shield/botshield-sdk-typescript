...
import { APIPromise } from '@botshield/botshield-sdk/core/api-promise';

import util from 'node:util';
import BotShield from '@botshield/botshield-sdk';
import { APIUserAbortError } from '@botshield/botshield-sdk';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['BOT_SHIELD_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: BotShield) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new BotShield({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('default logLevel is warn', async () => {
      const client = new BotShield({ apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new BotShield({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['BOT_SHIELD_LOG'] = 'debug';
      const client = new BotShield({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['BOT_SHIELD_LOG'] = 'not a log level';
      const client = new BotShield({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'BOT_SHIELD_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['BOT_SHIELD_LOG'] = 'debug';
      const client = new BotShield({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new BotShield({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['BOT_SHIELD_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new BotShield({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['BOT_SHIELD_BASE_URL'] = 'https://example.com/from_env';
      const client = new BotShield({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['BOT_SHIELD_BASE_URL'] = ''; // empty
      const client = new BotShield({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.botshield.ai/operations');
    });

    test('blank env variable', () => {
      process.env['BOT_SHIELD_BASE_URL'] = '  '; // blank
      const client = new BotShield({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.botshield.ai/operations');
    });
    ...
    test('env variable with environment', () => {
      ...
      expect(
        () => new BotShield({ apiKey: 'My API Key', environment: 'production' }),
      ...
      const client = new BotShield({ apiKey: 'My API Key', baseURL: null, environment: 'production' });
      expect(client.baseURL).toEqual('https://api.botshield.ai/operations');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new BotShield({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['BOT_SHIELD_BASE_URL'] = 'http://localhost:5000/env';
      const client = new BotShield({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new BotShield({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new BotShield({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['BOTSHIELD_API_KEY'] = 'My API Key';
    const client = new BotShield();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['BOTSHIELD_API_KEY'] = 'another My API Key';
    const client = new BotShield({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new BotShield({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new BotShield({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new BotShield({ apiKey: 'My API Key', fetch: testFetch });
