...
import { BotShieldDefaultError } from "../models/errors/bot-shield-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | BotShieldDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | BotShieldDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new BotShieldDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new BotShieldDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
